...
@interface PEURLSessionTaskInfo : NSObject
...
@interface PEURLSessionTaskInfo ()
...
@implementation PEURLSessionTaskInfo
...
@interface PEURLSession () <NSURLSessionDataDelegate>
...
@implementation PEURLSession


+ (PEURLSession *)sharedSession {
    static dispatch_once_t sOnceToken;
    static PEURLSession * session;
    dispatch_once(&sOnceToken, ^{
        NSURLSessionConfiguration *     config;
        
        config = [NSURLSessionConfiguration defaultSessionConfiguration];
        config.protocolClasses = @[ self ];
        session = [[PEURLSession alloc] initWithConfiguration:config];
    });
    return session;
}
...
- (instancetype)initWithConfiguration:(NSURLSessionConfiguration *)configuration
{
    ...
    if (self != nil) {
        ...
        [self->_sessionDelegateQueue setName:@"PEURLSession"];
        ...
        self->_session.sessionDescription = @"PEURLSession";
    }
    return self;
}
...
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request delegate:(id<NSURLSessionDataDelegate>)delegate modes:(NSArray *)modes
{
    ...
    taskInfo = [[PEURLSessionTaskInfo alloc] initWithTask:task delegate:delegate modes:modes];
...
- (PEURLSessionTaskInfo *)taskInfoForTask:(NSURLSessionTask *)task
{
    PEURLSessionTaskInfo *    result;
